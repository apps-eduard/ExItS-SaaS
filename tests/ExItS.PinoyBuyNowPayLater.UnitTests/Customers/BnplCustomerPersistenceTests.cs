...
public sealed class BnplCustomerPersistenceTests : IAsyncLifetime
{
    ...
    public async Task Migration_round_trip_enforces_org_isolation_and_unique_links()
    {
        ...
        await using (var scope = provider.CreateAsyncScope())
        {
            ...
            Assert.Contains("BnplInstallmentPlanRecord", entityTypes);
            Assert.Contains("BnplInstallmentPlanItemRecord", entityTypes);
            Assert.DoesNotContain(entityTypes, n => n.Contains("Repayment", StringComparison.OrdinalIgnoreCase));
            Assert.DoesNotContain(entityTypes, n => n.Contains("OutstandingBalance", StringComparison.OrdinalIgnoreCase));
