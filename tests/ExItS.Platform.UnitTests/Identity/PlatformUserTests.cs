...
public sealed class PlatformUserTests
{
    ...
    [Fact]
    public void Create_valid_user_normalizes_email_and_sets_active()
    {
        var user = PlatformUser.Create("Ada Lovelace", "Ada.Lovelace@Example.COM", T0);
        ...
        Assert.Equal(T0, user.CreatedAtUtc);
        Assert.Equal(T0, user.UpdatedAtUtc);
    }
    ...
    {
        var ex = Assert.Throws<DomainException>(() => PlatformUser.Create(name, "a@b.co", T0));
    ...
    {
        var ex = Assert.Throws<DomainException>(() => PlatformUser.Create("Ada Lovelace", email, T0));
    ...
    {
        var user = PlatformUser.Create("Ada Lovelace", "ada@example.com", T0);
        user.Suspend(T1);
    ...
    public void Rejects_non_utc_timestamp()
    {
        ...
        var ex = Assert.Throws<DomainException>(() => PlatformUser.Create("Ada Lovelace", "ada@example.com", local));
