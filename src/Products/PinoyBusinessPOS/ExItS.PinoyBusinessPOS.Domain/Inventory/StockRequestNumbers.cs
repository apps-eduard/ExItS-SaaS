...
public static partial class StockRequestNumbers
{
    ...
    public static string Format(DateOnly businessDate, long sequence)
    {
        if (sequence is < 1 or > MaxSequence)
        {
            throw new DomainException(
                DomainErrorCodes.InvalidStockRequestNumber,
                $"Stock request sequence must be between 1 and {MaxSequence} for a single business date.");
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(stockRequestNumber))
        {
            throw new DomainException(
                DomainErrorCodes.InvalidStockRequestNumber,
                "Stock request number is required.");
        }

        var trimmed = stockRequestNumber.Trim().ToUpperInvariant();
        if (trimmed.Length > MaxLength || !ValidPattern.IsMatch(trimmed))
        {
            throw new DomainException(
                DomainErrorCodes.InvalidStockRequestNumber,
                "Stock request number must look like SR-YYYYMMDD-NNNNNN.");
        }
    ...
    [GeneratedRegex(@"^SR-\d{8}-\d{6,}$", RegexOptions.CultureInvariant)]
