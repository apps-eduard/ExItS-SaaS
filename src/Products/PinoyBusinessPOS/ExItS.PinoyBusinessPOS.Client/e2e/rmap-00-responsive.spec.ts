...
async function assertCartPrimitives(page: Page, cart: "phone" | "landscape") {
  if (cart === "phone") {
    ...
    await expect(sheet.getByTestId(`sell-cart-qty-${MOCK_COKE_PRODUCT_ID}`)).toBeVisible();
  ...
  await expect(landscape.getByTestId(`sell-cart-qty-${MOCK_COKE_PRODUCT_ID}`)).toBeVisible();
...
test.describe("RMAP-00 shared responsive sell surface", () => {
  ...
  for (const viewport of viewports) {
    test(`${viewport.name} SearchField QuantityStepper MoneyDisplay StickyActionBar no overflow`, async ({
      page,
    }) => {
      await mockBoundCashierSession(page);
