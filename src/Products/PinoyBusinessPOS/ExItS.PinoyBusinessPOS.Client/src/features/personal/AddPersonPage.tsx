...
import { rememberResolvedPublicId } from "@/features/personal/people-status";
...
export function AddPersonPage() {
  ...
  async function onConfirmAdd() {
    ...
    setError(null);
    try {
      // Create contact with display name only — resolved identity is not linked by this API.
      const contact = await createMutation.mutateAsync({
        displayName: resolved.displayName,
      });
      rememberResolvedPublicId(contact.id, resolved.publicUserId);
  ...
  return (
    <section className="mx-auto flex w-full max-w-lg flex-col gap-4">
      <PageHeader title={t("people.addPerson")} />
      <p className="m-0 text-muted">{t("people.add.lede")}</p>
