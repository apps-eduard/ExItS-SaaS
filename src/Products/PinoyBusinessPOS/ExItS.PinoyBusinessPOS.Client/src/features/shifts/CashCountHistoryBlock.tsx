...
export function CashCountHistoryBlock({ label, amount, lines, testId, breakdownLabel }: Props) {
  const { t } = useI18n();
  const [expanded, setExpanded] = useState(false);
  const hasLines = Boolean(lines && lines.length > 0);
  ...
  return (
    <div data-testid={testId} className="flex flex-col gap-2">
      <button
        type="button"
        className="flex min-h-11 w-full items-center justify-between gap-2 rounded-[var(--exits-radius-md)] border border-border bg-surface px-3 text-left"
          ...
          {lines!.map((line) => {
            ...
            return (
              <li
                ...
                className="grid grid-cols-3 gap-2 rounded-[var(--exits-radius-md)] border border-border px-3 py-2 text-[length:var(--exits-text-sm)] tabular-nums"
              >
