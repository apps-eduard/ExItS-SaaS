...
function SocialAuthButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      className={cn(
        "flex min-h-11 flex-1 min-w-0 items-center justify-center gap-2 rounded-[var(--exits-radius-md)] border border-border bg-surface px-3 text-[length:var(--exits-text-sm)] font-semibold text-foreground transition-colors hover:bg-[var(--exits-surface-muted)] disabled:cursor-not-allowed disabled:opacity-60",
        className,
      )}
      onClick={onClick}
    >
...
export function SignInPage() {
  ...
  return (
    ...
    >
      ...
      {info ? (
        <p className="m-0 text-[length:var(--exits-text-sm)] text-success" data-testid="auth-info">
          {info}
        </p>
      ) : null}
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3" data-testid="auth-social-row">
        <SocialAuthButton
