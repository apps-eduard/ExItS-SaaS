...
internal sealed class PlanRepository : IPlanRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        query = (sortBy, sortDescending) switch
        {
            ...
            (CatalogListSortBy.MonthlyPrice, false) => query.OrderBy(p => p.MonthlyPrice).ThenBy(p => p.Code),
            (CatalogListSortBy.MonthlyPrice, true) => query.OrderByDescending(p => p.MonthlyPrice).ThenBy(p => p.Code),
            (CatalogListSortBy.AnnualPrice, false) => query.OrderBy(p => p.AnnualPrice).ThenBy(p => p.Code),
            (CatalogListSortBy.AnnualPrice, true) => query.OrderByDescending(p => p.AnnualPrice).ThenBy(p => p.Code),
            (CatalogListSortBy.CurrencyCode, false) => query.OrderBy(p => p.CurrencyCode).ThenBy(p => p.Code),
            (CatalogListSortBy.CurrencyCode, true) => query.OrderByDescending(p => p.CurrencyCode).ThenBy(p => p.Code),
