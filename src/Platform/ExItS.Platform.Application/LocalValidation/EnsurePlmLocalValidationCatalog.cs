...
/// <summary>
/// Idempotent Local Validation-only catalog and commercial fixture for Pinoy Loan Manager.
...
/// PLM-D-00-02/04–08/11–13, pricing, or grant-identifier decisions (PLM-D-00-06 remains open).
/// </summary>
public sealed class EnsurePlmLocalValidationCatalog
{
    public const string ProductDisplayName = "Pinoy Loan Manager";
    ...
    public EnsurePlmLocalValidationCatalog(
    ...
    public async Task EnsureReferenceAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (product is null)
        {
            var created = await _createProduct
                .ExecuteAsync(ProductCode.PinoyLoanManager, ProductDisplayName, cancellationToken)
            ...
            if (!created.IsSuccess)
            {
                ...
                if (product is null)
                {
                    throw new InvalidOperationException(
                        $"Local validation PLM product create failed: {created.ErrorCode} {created.ErrorMessage}");
                }
            }
            else
            {
        ...
        var planCode = PlanCode.Create(LocalValidationOptions.PlmLocalValidationPlanCode);
        ...
        if (plan is null)
        {
            var createdPlan = await _createPlan
                .ExecuteAsync(
                    ...
                    LocalValidationOptions.PlmLocalValidationPlanCode,
                    LocalValidationOptions.PlmLocalValidationPlanDisplayName,
                    cancellationToken)
                .ConfigureAwait(false);
            if (!createdPlan.IsSuccess || createdPlan.Value is null)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Local validation PLM plan create failed: {createdPlan.ErrorCode} {createdPlan.ErrorMessage}");
                }
            }
            else
            {
        ...
        if (plan.Status == PlanStatus.Draft)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Local validation PLM plan activate failed: {activated.ErrorCode} {activated.ErrorMessage}");
            }
        ...
        if (published is null)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Local validation PLM plan version publish failed: {publishedResult.ErrorCode} {publishedResult.ErrorMessage}");
            }
        }
        ...
        var trial = trials.FirstOrDefault(t =>
            string.Equals(
                t.DisplayName,
                LocalValidationOptions.PlmLocalValidationTrialDisplayName,
        ...
        if (trial is null)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Local validation PLM trial create failed: {createdTrial.ErrorCode} {createdTrial.ErrorMessage}");
            }
        }
    }

    /// <summary>
    /// Starts an independent PLM trial subscription and entitlement snapshot for one organization.
    /// Does not reuse POS subscription, snapshot, or product-access assignment.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Local validation PLM plan '{LocalValidationOptions.PlmLocalValidationPlanCode}' was not available.");
        }
        ...
        if (version is null)
        {
            throw new InvalidOperationException("Published Local Validation PLM plan version was not available.");
        }
        ...
        if (trial is null)
        {
            throw new InvalidOperationException("Local validation PLM trial definition was not available.");
        }
        ...
        if (subscription is null)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Local validation PLM trial subscription failed: {started.ErrorCode} {started.ErrorMessage}");
            }
        }
        ...
        {
            throw new InvalidOperationException(
                $"Local validation PLM entitlement snapshot failed: {snapshot.ErrorCode} {snapshot.ErrorMessage}");
        }
    }
}
