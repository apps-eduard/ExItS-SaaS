...
Physical/user confirmation still required (P18-WP08). Prefer PhysicalDevice / Tailscale Local Validation. Emulator Debug (secondary) uses `http://10.0.2.2:8091` / `:8092`; use an ExItS-named AVD.
...
- PIN keypad beside Google when PIN can be offered (online or offline). Compact offline-only **Use PIN** text link removed. Debug Local Validation may still treat `NetworkAccess.None` as connected for API attempts.
