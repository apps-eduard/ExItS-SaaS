...
**portfolio independence verification:** Held (read-only; not tracked by root Git)
...
legacy product is suitable for **controlled platform extraction** of identity, organization, permission, audit, and cross-cutting API patterns — not a wholesale move. New ExITS Platform Admin and PinoyBusinessPOS share a **native** CSS/Razor UI foundation (**no Ant Design**, **no Tailwind**). Existing legacy product Staff Web retains Ant Design. Plans/billing/entitlements are **missing** and must be built on the Platform. Next work: **P1-WP01 — Platform vs Product Capability Boundary** (documentation/architecture only; no legacy product import; no application skeleton yet unless Phase 1 explicitly expands).
...
Root remote remains **empty** (`origin` → `ExItS-SaaS`, `origin/main` gone). Nested `legacy product/.git` preserved and ignored.
...
| **Keep exclusively in legacy product** | Clinics, clinical staff roles, Patients, patient self-scope, Appointments, availability, reminders, medical notes/amendments, clinical permissions/audit, PatientWeb/Mobile clinical UX, Ant Design Staff Web |
...
| **Deferred** | MFA, production email, EF global tenant filters, rich calendar, Full POS modules, iOS shipping validation, legacy product Ant modernization, legacy product monorepo import |
...
**legacy product:** clinics, clinical workforce, patients, appointments, notes, clinical authz/audit, foreign product workflows.
...
| Surface | Stack |
|---|---|
| legacy product Staff Web | Ant Design Blazor (retain; no rewrite now) |
| legacy product PatientWeb / MAUI | Existing native implementations (retain) |
...
| Option | Verdict |
|---|---|
| Controlled monorepo import of legacy product | **Later** — after Platform foundation + extraction plan; preserves history carefully |
...
**Recommended next step (Phase 1 start):** Keep the repository independent with no nested foreign product source tree. **Build new Platform documentation and, when Phase 1 authorizes code, new Platform foundations in the root repository without importing external product source.** Do not create foreign product submodules in Phase 0/early Phase 1.
...
| Organizations, memberships | Platform | legacy product may keep product projections |
| Products/plans/subs/payments/entitlements | Platform | Greenfield |
| Platform audit | Platform | |
| Clinics, patients, appointments, notes | legacy product | |
| Clinical audit detail | legacy product | |
...
- Platform roles ≠ product roles; patient self-scope legacy product-only; POS customer access ≠ patient self-scope
...
|---|---|
| R-001 legacy product regression | Phase 2 extraction + regression gates |
| R-002 legacy product rules becoming platform rules | Phase 1 boundary + matrix enforcement |
...
| R-015 nested dirty legacy product files | legacy product repo / import WP |
...
| Expected outputs | Updated/approved boundary docs, matrices, decisions — **documentation first**; no legacy product import |
| Explicit exclusions | No legacy product code changes; no POS app; no billing implementation; no UI components; no DB migrations; no monorepo import |
| Required tests | Doc/link validation; no legacy product test re-run required unless Phase 1 later expands |
