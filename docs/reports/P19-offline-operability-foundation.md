...
| Lock vs Sign out | **Lock** keeps grant + PIN and the active session slot (tokens not reused across users). **Sign out** drops process unlock and the bearer; durable grant + PIN remain; a per-user Platform session handle may recover online via PIN (`GrantType: session`) until the server session expires or is explicitly revoked. Next **online** password auth refreshes the grant without wiping same-user PIN |
