...
UI: Settings (online) for optional PIN change via the shared enrollment page; `/offline-pin-setup` for **mandatory** enrollment after online auth when complete setup is missing; `/offline-pin` for unlock **only when eligible**; Sign-in surfaces a compact **Use PIN** link when PIN is eligible **and** the OS reports no network interface (or a sign-in attempt failed as unreachable). Debug Local Validation may still treat `NetworkAccess.None` as connected for API attempts; PIN offer uses `HasNoNetworkInterfaceAsync` and must survive supported cold restart. Reconnect surfaces PIN offer when grant is valid. First-time offline devices never see a PIN field or invalid-PIN copy.
...
| Use PIN on Sign-in | Compact text link, shown when enrolled PIN + usable offline operate grant exist **and** the OS reports no network interface (or a sign-in attempt failed as unreachable). Not shown merely because a PIN exists while the radio reports a usable network. Debug Local Validation may still treat `NetworkAccess.None` as connected for API attempts; PIN offer uses `HasNoNetworkInterfaceAsync`. No large offline info panel on the login card. See [maui-auth-experience.md](../engineering/maui-auth-experience.md). |
