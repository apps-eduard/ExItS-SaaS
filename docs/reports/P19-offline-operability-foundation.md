...
UI: Settings (online) for optional PIN change via the shared enrollment page; `/offline-pin-setup` for **mandatory** enrollment after online auth when complete setup is missing; `/offline-pin` for unlock **only when eligible**; Sign-in shows a round keypad beside Google whenever ≥1 complete eligible PIN identity exists (online or offline). See [maui-auth-experience.md](../engineering/maui-auth-experience.md). First-time offline devices never see a PIN field or invalid-PIN copy.

## 6b. Auth UX layer (mandatory PIN + login readiness)

| Topic | Behavior |
|---|---|
| Mandatory PIN enrollment | After successful online auth (password, Platform session/profile, Local Validation/GUID where allowed), `EvaluateCurrentUserOfflinePinReadinessAsync` checks **complete** setup for the signed-in user: enrolled identity + valid device-bound grant + matching device + unexpired/non-revoked + PIN verifier. Missing setup → `/offline-pin-setup` (`/setup-pin` alias) before home. Already complete → existing destination/return route. Persistence failure stays on setup. |
| First-time offline | If `EvaluateOfflineColdStartOfferAsync` is not eligible: no PIN field, no `/offline-pin`, no **Invalid PIN** / **Incorrect PIN**. Sign In stays available. Password attempt while offline uses `SignIn_OfflineNoPinMessage` (internet required to sign in once and set up PIN). |
| Wrong PIN | Shown only when an enrolled verifier exists and the entered PIN is incorrect (plus existing lockout). Never map “not enrolled” to wrong PIN. |
| Forgot PIN | Offline: no local reset; reconnect and authenticate to change PIN. Online: Settings → shared `/offline-pin-setup?mode=change` (same `SetOfflinePinAsync` path). |
...
| Use PIN on Sign-in | Round keypad beside Google when a complete eligible PIN identity exists, online or offline. Accessible name **Sign in with PIN**. Helper copy `SignIn_PinKeypadHint`. Not shown when no eligible identity exists. Slow-login still offers **Use PIN instead**. See [maui-auth-experience.md](../engineering/maui-auth-experience.md). |
...
| Lock vs Sign out | **Lock** clears active in-memory authorization; preserves enrolled PIN, offline grant, and per-user device recovery credentials. **Sign out** drops process unlock and the bearer; durable grant + PIN + recovery credential remain until expiry or explicit revocation. Next online PIN exchange uses `POST /api/v1/platform/auth/recovery/exchange` (60-minute AccessToken, credential rotation). Expired/missing recovery → **Online verification required** (one full sign-in to re-enroll). Legacy `pos.pin.recovery.session.{userId}` may migrate once while online. |
...
Online provisioning: successful auth → `EnsureOfflineOperateGrantAsync` (POS device authorize/bind when needed, then grant) → readiness check → `/offline-pin-setup` if incomplete, or Settings PIN save → verifier. Local Validation / Development GUID login uses this same grant+verifier path; it does not invent a PIN or skip enrollment. Complete setup is identity + grant + matching device + verifier. Orphan verifier without grant is not eligible. Remembered/typed username is not authorization truth.
