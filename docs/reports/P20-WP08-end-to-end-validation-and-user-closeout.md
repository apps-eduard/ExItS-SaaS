...
| Suite | Result |
|---|---|
| Platform Unit — GlobalCatalog | **68 passed**, 0 failed |
...
- [ ] CSV bulk import: download template → fill → preview (new categories warned, not failed) → confirm creates categories + products → progress → partial errors visible
