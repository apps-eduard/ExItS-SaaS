...
|---|---|---|---|---|---|---|---|---|
| Identifier mismatch | 6 | Map User/Org incorrectly | Broken FK history / wrong actor | Preserve HC IDs; explicit mapping; no early FK rewrite | Mapping checksums; sample joins | L3 / L5 | Platform + HC | Yes for cutover |
...
| Organization mapping error | 3–6 | Wrong Platform↔HC org | Cross-tenant leakage | 1:N clinic mapping tests; server scope | Tenant denial tests | L3 | Platform + HC | **Critical** |
...
| Lost permission | 6 | Incomplete role map | Clinicians blocked | Permission matrix parity tests | Diff report vs baseline | L2 | HC product | Yes for cutover |
...
| HealthCare regression | 6 | Any HC change/cutover | Product outage | Freeze early; regression gate | 1102 + Integration/E2E | L2–L5 | HC product | **Critical** |
| Accidental HC root tracking | 1+ | `git add HealthCare` | Nested secrets/history leak | Ignore + pre-commit checks | `git ls-files HealthCare` | L0 revert | Repo lead | **Critical** |
...
| POS delayed by Platform overengineering | 1–5 | Waiting for full HC migration | Missed POS MVP | POS readiness gate without full HC reconnection | Schedule review | De-scope Platform extras | Portfolio lead | No for P1-WP03 |
