...
26. **P6-WP02:** Remarks-based credit entries are organization-scoped the same way. Append-only after create; corrections use explicit reversal with a required reason. Outstanding is derived from active entries only. SaaS subscription payments remain distinct from customer Utang credit. No repayments/ledger yet.
