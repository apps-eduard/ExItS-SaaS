...
28. **P6-WP04:** Credit due dates and overdue queries are organization-scoped; set/clear require Development/Testing `X-Dev-Platform-User-Id`. Due-date history is append-only. FIFO aging is derived (not persisted). Effective business date is server UTC calendar day only — org timezone not defined. No statements or printable receipts yet.
