...
|---|---|---|---|---|
| R-001 | legacy product regression during extraction | Critical | Baseline tag, tests, small extraction steps, rollback | Open |
| R-002 | legacy product-specific rules become generic platform rules | Critical | Classification matrix and architecture review | Open — matrix started in P0-WP01 |
...
| R-005 | Ant Design coupling spreads into POS **or new Platform Admin** | Medium | ADR-010: Ant only in existing legacy product Staff Web; native stack for Platform Admin + POS | **Mitigated** (strategy) — watch during Phase 4–5+ |
...
| R-007 | English-only strings escape into release | Medium | Resource-completeness tests; POS `en`/`fil` from MVP | Open — legacy product has no i18n; POS greenfield (P0-WP03) |
| R-008 | Dark theme creates poor contrast | High | Semantic tokens and accessibility tests; Light/Dark/System for POS | Open — legacy product lacks product theme switch; POS tokens planned (P0-WP03) |
| R-009 | Duplicate offline financial transactions | Critical | Idempotency and append-only ledger | Open |
| R-010 | Nested `legacy product/.git` inside ExITS monorepo | High | Nested product folder removed from ExItS workspace (P10-WP02 Part A); do not recreate without authorization | **Closed** (folder absent; independence tests updated) |
...
| R-014 | Full `legacy product solution` build fails without Android SDK env | Medium | Historical legacy product-only risk; ExItS builds independently | **Closed for ExItS workspace** (no nested legacy product tree) |
| R-015 | Pre-existing dirty PatientWeb files inside nested legacy product git | Medium | Historical legacy product-only risk; nested tree removed from ExItS | **Closed for ExItS workspace** |
...
| R-017 | Accidental legacy product parent tracking | Critical | No nested tree; architecture `RepositorySafetyTests` assert absence + no solution reference | **Mitigated** (tests + process) — keep verifying |
| R-018 | Nested legacy product local `.env` / lab secrets | High | Nested tree removed from ExItS; never recreate secrets into portfolio docs | **Closed for ExItS workspace** |
| R-019 | Dual UI stacks (legacy product Ant vs **native** Platform Admin + POS) — brand drift, duplicated visuals, separate a11y/theme work, future legacy product modernization cost | Medium | Shared semantic tokens, branding, terminology, UI-independent contracts; separate framework impls; no forced legacy product rewrite in current MVP | Open — **controlled technical separation** (ADR-010) |
...
| R-026 | Premature legacy product import or wholesale copy before Platform gates | Critical | ADR-013; P2-WP01 foundation only; legacy product not in solution | **Mitigated** (P2-WP01 evidence) — watch later WPs |
...
| R-028 | Phase 1 closed while Integration/E2E and numeric entitlement windows remain open | Medium | Documented deferred; re-baseline before legacy product cutover; R-022 for durations | Open — accepted with Phase 1 closeout |
...
| R-036 | Contract major-version incompatibility between Platform and legacy product consumers | High | Fail closed on unsupported majors; version negotiation later | Open — introduced P2-WP04 |
...
| R-040 | Premature assumption that contracts equal completed legacy product integration | High | Report states foundation only; portfolio independence verification continues | Open — introduced P2-WP04 |
