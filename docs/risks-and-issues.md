...
|---|---|---|---|---|
| R-001 | Platform or product regression during structural change | Critical | Baseline, tests, focused work packages, rollback | Open |
| R-002 | Product-specific rules become generic Platform rules | Critical | Capability ownership and architecture review | Open |
...
| R-005 | Ant Design coupling spreads into POS | Medium | ADR-010/015: Platform Admin owns Ant; POS uses native DesignSystem | **Mitigated** (strategy) |
| R-006 | Native reusable components become a full UI-framework project | High | Phase-gated catalog (MVP/Utang/Store/Full); build only phase-needed components | Open — catalog in reusable-component-catalog |
| R-007 | English-only strings escape into release | Medium | Resource-completeness tests; POS `en`/`fil` from MVP | Open |
| R-008 | Dark theme creates poor contrast | High | Semantic tokens and accessibility tests; Light/Dark/System | Open |
| R-009 | Duplicate offline financial transactions | Critical | Idempotency and append-only ledger | Open |
| R-010 | Unapproved nested repository inside ExITS monorepo | High | Repository safety tests and work-package authorization | **Closed** |
| R-011 | No EF global tenant query filters (service-only isolation) | Critical | Keep service checks; add filters/tests before multi-product sharing | Open — verified in early portfolio safety inventory |
...
| R-013 | Parent repo missing root `.gitignore` | High | Root `.gitignore` added | **Mitigated** |
| R-014 | Full mobile solution build requires Android SDK environment | Medium | Document and validate supported build agents | **Mitigated** |
| R-015 | Nested-repository changes could evade root status checks | Medium | Repository safety tests and root ownership rules | **Closed** |
...
| R-017 | Accidental tracking of unapproved product sources | Critical | Architecture `RepositorySafetyTests` and solution inventory checks | **Mitigated** |
| R-018 | Local environment secrets enter source control | High | Ignore rules, secret checks, and review | **Mitigated** |
| R-019 | Distinct Admin and POS UI stacks drift in brand or accessibility | Medium | Shared semantic tokens, terminology, and UI-independent contracts | Open — **controlled technical separation** |
| R-020 | Integration/E2E validation differs across build environments | Medium | Run suites on approved agents and record exact evidence | Open |
...
| R-026 | Unapproved product source or wholesale copy enters the portfolio | Critical | Repository boundaries and explicit work-package approval | **Mitigated** |
| R-027 | Production migration without restore rehearsal or rollback evidence | Critical | Require backup, restore, and rollback rehearsal before migration | Open |
| R-028 | Architecture closes while integration and entitlement windows remain open | Medium | Track deferred validation and R-022 durations | Open |
...
| R-036 | Contract major-version incompatibility between Platform and product consumers | High | Fail closed on unsupported majors; version negotiation later | Open — introduced P2-WP04 |
...
| R-038 | Organization mapping errors across product projections | High | Explicit reversible mapping contracts; no destructive ID rewrite | Open — introduced P2-WP04 |
| R-039 | Platform organization roles escalate into product-operational roles | Critical | Contracts exclude product roles; docs + architecture tests | Open — introduced P2-WP04 |
| R-040 | Contract interfaces mistaken for completed product integration | High | Reports state foundation-only status | Open — introduced P2-WP04 |
...
P9-WP04 hardened accessibility/localization/theme QA: Admin page chrome localized EN/fil-PH; StatusBadge text+marker; skip links; Dialog/ConfirmDialog labelled-by + focus; MoneyDisplay accessible labels; DesignSystem contrast token tests; culture fallback and resource parity guards. Interactive TalkBack/device theme walkthrough remains R-109. RTL unsupported. **Not production-ready.** Superseded as "next WP" by P9-WP05.
...
P8-WP02 delivered organization-isolated simple retail sales (`pos.sales`, `pos.sale_lines`, `pos.sale_number_sequences`; migration `AddPosSimpleSales`). Cash and ManualGCash; server-authoritative totals; AwayFromZero 2-dp rounding; idempotent checkout; explicit void. Features `store-sales-view` / `store-sales-create` / `store-sales-void`. Superseded as “next WP” by P8-WP03.
...
Phase 7 Offline Synchronization is **closed**. P7-WP05 reconciled WP01–WP04, hardened confirmed defects (projection rebuild, encryption of pending due-date/reversal fields, safe conflict JSON, per-operation capability revalidation, RecoveryRequired UX), and documented the recovery matrix. R-109 remains open. R-022 remains open. R-129 / full-DB encryption remains an explicit production gate. OD-10 retained. **Not production-ready.** Phase 8 started with P8-WP01.

## Phase 7 note (P7-WP04)

P7-WP04 delivered encrypted local repayment projections and offline `RepaymentCreate`, `RepaymentReverse`, `CreditReverse`, `CreditDueDateSet`, and `CreditDueDateClear` via the generic queue. Row-level AES-GCM chosen; SQLCipher deferred (R-129 not worsened). **No offline statements/receipts.** R-109 remains open. R-022 remains open. OD-10 retained. Superseded as “next WP” by P7-WP05.
