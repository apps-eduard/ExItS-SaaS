...
| R-010 | Nested `HealthCare/.git` inside ExITS monorepo | High | Nested product folder removed from ExItS workspace (P10-WP02 Part A); do not recreate without authorization | **Closed** (folder absent; independence tests updated) |
...
| R-014 | Full `HealthCare.sln` build fails without Android SDK env | Medium | Historical HC-only risk; ExItS builds independently | **Closed for ExItS workspace** (no nested HC tree) |
| R-015 | Pre-existing dirty PatientWeb files inside nested HealthCare git | Medium | Historical HC-only risk; nested tree removed from ExItS | **Closed for ExItS workspace** |
...
| R-017 | Accidental HealthCare parent tracking | Critical | No nested tree; architecture `RepositorySafetyTests` assert absence + no solution reference | **Mitigated** (tests + process) — keep verifying |
| R-018 | Nested HealthCare local `.env` / lab secrets | High | Nested tree removed from ExItS; never recreate secrets into portfolio docs | **Closed for ExItS workspace** |
...
| R-091 | Missing production authentication (JWT / passwords / MFA / SSO / AD) | Critical | Server-side `PlatformAuthz` + role assignments for development; **P13-WP02**–**P13-WP07** credentials/session/password lifecycle/org context/product-client bearer/MFA readiness+hardening; require Phase 13 closeout before production; no fake login; MFA enforcement deferred | Open — introduced P4-WP04; production blocker; Phase 13 in progress |
...
**P13-WP07** added MFA readiness contracts/DTO signals (non-enforcing), suspend/deactivate session+token revoke, token-ops/email-verification rate limits, Production MFA-flag and access-token lifetime guards, Admin/POS HTTPS BaseUrl guards. **No MFA enrollment/challenge / IdP.** **R-091 remains open.** Exact next: **P13-WP08 — Phase 13 Closeout** (do not begin). **Not production-ready.**
