...
| R-091 | Missing production authentication (JWT / passwords / MFA / SSO / AD) | Critical | Server-side `PlatformAuthz` + role assignments for development; **P13-WP02**–**P13-WP06** credentials/session/password lifecycle/org context/product-client bearer; require MFA readiness + closeout before production; no fake login | Open — introduced P4-WP04; production blocker; Phase 13 in progress |
...
**P13-WP06** added Platform opaque Bearer access tokens, Admin product-entry exchange, MAUI password login with Bearer attachment, and POS API introspection (identity + org + product access) before product-local roles. Dev GUID/headers remain Dev/Testing-only. **No MFA / IdP.** **R-091 remains open.** Exact next: **P13-WP07 — MFA Readiness and Auth Hardening** (do not begin). **Not production-ready.**
