...
1. Preserve the completed legacy product MVP in a working state.
2. Create a baseline tag or commit before structural extraction.
3. Never copy legacy product migrations into PinoyBusinessPOS.
4. Do not rename legacy product concepts into POS concepts.
5. Platform extraction requires contract tests and legacy product regression tests.
...
9. Ant Design Blazor remains only in existing legacy product Staff Web. New ExITS Platform Admin and PinoyBusinessPOS use native CSS/Razor — Ant is not required and must not be introduced there.
...
13. Build **new** Platform foundations in root Git before legacy product reconnection (ADR-013). Do not wholesale-copy the legacy product solution. Do not import legacy product until an approved WP.
14. POS may begin after the POS readiness gate without waiting for full legacy product cutover.
