...
Define the safe, incremental sequence for building the new ExITS Platform from reusable legacy product **patterns** without modifying, importing, or destabilizing the existing legacy product application during early phases.
...
| legacy product nested, ignored, read-only | Required continuously |
| Windows-safe legacy product baseline 1102/0/0 | Recorded (P0-WP02); re-verify before legacy product cutover |
| Integration/E2E environment available | Required before legacy product cutover (R-020) |

## 3. New-build versus extraction decision

**Decision (ADR-013):** Create the Platform foundation **new** in the ExITS root repository. Adapt verified legacy product patterns selectively. Do **not** initially copy the complete legacy product solution. Do **not** import legacy product into root Git until an approved later WP.

### Classification of reusable areas

| Area | Classification |
|---|---|
| Identity (global users) | Reimplement from approved pattern (legacy product ApplicationUser → Platform user) |
| Authentication / JWT | Reimplement from approved pattern; legacy product unchanged until reconnection |
| Refresh tokens | Reimplement from approved pattern (rotation/family revoke) |
| Organizations | Reimplement as PlatformOrganization; map legacy product orgs later |
...
| Product operational roles | Keep exclusively in legacy product / POS |
...
| Audit (security/org) | Adapt pattern for Platform; clinical audit stays in legacy product |
...
| Background jobs | Adapt Hangfire *hosting* pattern; do not share legacy product job DB/workers |
...
| Clinical domain (Patient, Clinic, notes, appointments) | Keep exclusively in legacy product |
| Ant Design Blazor / legacy product Staff CSS | Keep exclusively in legacy product Staff Web |
| Patient self-scope | Keep exclusively in legacy product |
| Password hashes / wholesale Identity DB copy | **Do not reuse** without separately approved migration plan |
| legacy product migrations history | **Do not reuse** / do not copy into Platform |
...
Future: root solution, Platform projects, test projects, build conventions, dependency rules. **No legacy product import.** Gate: architecture tests green; `git ls-files legacy product` empty.

### Stage 2 — Platform identity foundation

Future: global users, authentication, refresh tokens, sessions, security events. **No legacy product authentication change.** Gate: identity tests; no credential leakage.
...
### Stage 6 — legacy product adapter / reconnection preparation

Only after Platform behavior is proven: identity mapping, organization mapping, contract adapters, migration dry runs, compatibility testing. **legacy product remains unchanged until an approved Phase 2+ WP.** Gate: mapping reports; legacy product regression (1102+); Integration/E2E in supported env; rollback rehearsal.

### Stage 7 — PinoyBusinessPOS foundation

Begin only after POS readiness gate (§13). Does **not** require full legacy product reconnection. Gate: contracts stable; Cash/GCash/Utang requirements intact.
...
**Prohibit:** product → Platform EF entities/DbContext; Platform → POS or legacy product domain; Shared → product projects; UI library → product business entities; cyclic references.

Architecture tests required later to enforce these rules.

## 6. Identity continuity

- Preserve existing legacy product user IDs; do not rewrite clinical FKs in early phases.
- Introduce explicit, reversible, auditable mapping to PlatformUserId where needed.
- No destructive identifier migration.
- Do not assume legacy product IDs can be replaced safely.
...
- Existing legacy product Organization remains authoritative for legacy product until approved cutover.
...
- Platform product access ≠ legacy product operational roles ≠ POS operational roles.
- legacy product permission behavior unchanged until approved reconnection.
- Prove: no privilege escalation; no lost clinical permissions; no cross-org access; Platform admins do not gain clinical access; concealment retained; patient self-scope stays legacy product-local.

## 9. Database sequence

Target databases (future): `ExItS_Platform` / `ExItS_legacy product` / `ExItS_PinoyBusinessPOS`.

Initial legacy product database **unchanged**.
...
**Prohibit:** editing old legacy product migrations; copying migration history into Platform; cross-DB FKs; destructive migration without tested restore; automatic cascading deletion across databases.

## 10. Testing gates

See [implementation-gate-matrix.md](../engineering/implementation-gate-matrix.md).

Summary: foundation (architecture, build, unit) → identity → tenant → contract → migration → legacy product regression (1102 baseline; Integration/E2E before cutover).
...
- Platform deploys independently; legacy product stays on existing deployment initially.
...
POS does **not** need full legacy product reconnection first.

## 14. Explicit exclusions (this WP and early stages)

- No legacy product file changes or import
- No wholesale legacy product solution copy
...
| OD-05 | legacy product import timing | Portfolio lead | After Platform foundation | No | Keep nested ignored |
| OD-06 | Multi-org from legacy product StaffMember | Platform + legacy product | Phase 2 | No | legacy product single membership until cutover WP |
