...
- Organization Owner Personal membership grants **Organization Web management** for that Organization. It does **not** automatically grant a POS product-local checkout role (`CreateSale` / `EnterPos`). POS management APIs may use `OrganizationManagementAuthority` when commercial entitlement is active. See [organization-web-role-and-workflow-matrix.md](../engineering/organization-web-role-and-workflow-matrix.md).
...
Buyer↔supplier relationships remain **organization↔organization** commercial links. Public identity QR does not create supplier relationships. Connected-supplier connect requires Business QR / `ORG######` and rejects Personal and device-registration QR on both MAUI and POS Application. Relationship ownership is org↔org (survives ownership transfer); actor user id is audit only. Request lifecycle: Pending → Active/Declined; buyer Pending and supplier Incoming must both be visible in product UI. **Connected buyer ≠ Customer** — accepting a connection never auto-creates a seller-owned Customer. Organization business notifications for connection events are Organization-scoped (not Personal-owned) via Platform `OrganizationInAppNotification` (tap marks Read; Unread ≠ Pending Action). See [connected-exits-suppliers.md](../engineering/connected-exits-suppliers.md), [connected-supplier-connection-request-lifecycle.md](../reports/connected-supplier-connection-request-lifecycle.md), and [unified-organization-business-notifications.md](../reports/unified-organization-business-notifications.md).
