...
Phase 1 produced capability boundaries, contracts, payment MVP rules, delivery sequencing, and rollback requirements. Implementation agents need explicit approval to begin narrow foundation work without ambiguity or portfolio-boundary violations.

## Decision

1. **Phase 1 architecture is approved** (close with documented risks).
2. **New Platform is built in root Git**; no unauthorized nested product source tree is permitted without an approved work package.
...
8. **All later capability work remains work-package controlled**; no wholesale copy from an external product tree and no large shared library without two consumers.
...
- Import an unauthorized external product source tree before Platform foundation gates.
