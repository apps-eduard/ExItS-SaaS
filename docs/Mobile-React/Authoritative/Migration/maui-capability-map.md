...
|------------|-----------------|-------------------|---------------|----------------|-------|---------------------|--------|----------|----------------------|
| Sign-in (email + staff login) | `/signin` | `SignIn.razor`, `IAuthenticationService` | `/api/v1/platform/auth/login` | OfflineCapable shell | n/a | Real email or `local@ORG######` staff principal | PROVEN_CURRENT | Auth pages | Formal person-link is not a shared login |
...
| Personal staff-invite accept | `/personal/invitation-accept` | `PersonalInvitationAccept.razor` | anonymous accept **or** `accept-as-personal` | OnlineRequired | Personal or token | Sets staff password; shows staff login | PROVEN_CURRENT | | React RMAP-01b |
| Staff invite/assign | `/org/staff*` | Staff pages | staff-invitations + roles | OnlineRequired | Owner/Admin | Invite → accept creates staff identity; Personal session uses accept-as-personal | PROVEN_CURRENT | | React MISSING (RMAP-01b) |
