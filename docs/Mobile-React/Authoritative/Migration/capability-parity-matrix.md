...
| Today’s Prices | Daily price change | PROVEN_CURRENT | PROVEN_CURRENT | PROVEN_CURRENT | CURRENT | CURRENT | Catalog admin, RMAP-06 | Override UI later (RMAP-12b) | prices endpoint; validation closeout `cb91145b` |
| Commercial sale discount | Preserve UnitPrice; separate adjustment | PROVEN_CURRENT | PROVEN_CURRENT (no discount UI) | PROVEN_CURRENT (quote + checkout UX) | CURRENT | CURRENT (RMAP-11b) | RMAP-B03 | — | ApplyCommercialDiscount; quote+checkout |
| Sale-line price override | Role-gated per-sale UnitPrice; never catalog | PROVEN_CURRENT | PROVEN_PARTIAL | MISSING (UI) | CURRENT (backend) | BLOCKED on RMAP-12b | RMAP-B01 | React override UX | Cashier DENY; Manager ≤100%; Owner unlimited; CashierAdjustable SUPERSEDED |
