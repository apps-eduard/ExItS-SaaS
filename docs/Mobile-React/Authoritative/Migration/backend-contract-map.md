...
| Org-scoped staff login | Platform | `PlatformUser` staff | Invite accept + `StaffLoginNameRules` | `/organization-invitations/accept`, staff-invitations | `platform_users.normalized_email` | Org invite token | Unique email | `OrganizationScopedStaffIdentityTests` | PROVEN_CURRENT | StaffLoginNameRules.cs |
