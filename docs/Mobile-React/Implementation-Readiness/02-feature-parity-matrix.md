...
| Connected suppliers | Request / buyers / incoming / QR request / seller invite-buyer | `/suppliers/connected*` `/customers?kind=businesses` `/customers/new/business/organization` | Connected-supplier client; QR scan; `InitiatedByParty` consent | Role-gated ManageSuppliers for invite/respond; CreateSale for checkout Active/Pending list | Mostly OnlineRequired; linked-products OfflineCapable; draft Queueable | Selective local cache | Camera/QR | Connected suppliers | Feasible-degrade | QR camera | Yes | Yes | Phone | API | Yes for QR | READY for consent lifecycle (POS-B2B-BUSINESS-CONNECTION-CONSENT-LIFECYCLE-01) | QR request: CAPACITOR_REQUIRED for camera parity |
